...
class ASRProviderBase(ABC):
    ...
    async def speech_to_text_wrapper(
        ...
        try:
            ...
            if audio_format == "wav":
                # 处理WAV格式：从WAV文件中提取PCM数据
                for wav_file in opus_data:
                    # 检查WAV文件头，支持正常和小端字节序
                    is_wav = False
                    if wav_file.startswith(b'RIFF') and wav_file[8:12] == b'WAVE':
                        # 大端字节序WAV文件
                        is_wav = True
                    elif wav_file.startswith(b'FFIR') and wav_file[8:12] == b'EVAW':
                        # 小端字节序WAV文件，需要反转字节序
                        is_wav = True
                        logger.bind(tag=TAG).info("检测到小端字节序WAV文件，正在转换...")
                        # 反转整个文件的字节序
                        import struct
                        # 读取并反转WAV文件头
                        riff_id = wav_file[0:4][::-1]
                        file_size = struct.unpack('<I', wav_file[4:8])[0]
                        wave_id = wav_file[8:12][::-1]
                        fmt_chunk_id = wav_file[12:16][::-1]
                        fmt_chunk_size = struct.unpack('<I', wav_file[16:20])[0]
                        audio_format = struct.unpack('<H', wav_file[20:22])[0]
                        num_channels = struct.unpack('<H', wav_file[22:24])[0]
                        sample_rate = struct.unpack('<I', wav_file[24:28])[0]
                        byte_rate = struct.unpack('<I', wav_file[28:32])[0]
                        block_align = struct.unpack('<H', wav_file[32:34])[0]
                        bits_per_sample = struct.unpack('<H', wav_file[34:36])[0]
                        data_chunk_id = wav_file[36:40][::-1]
                        data_chunk_size = struct.unpack('<I', wav_file[40:44])[0]
                        pcm_data_bytes = wav_file[44:]
                        
                        # 重新构建大端字节序的WAV文件
                        new_wav_file = b''
                        new_wav_file += riff_id
                        new_wav_file += struct.pack('>I', file_size)
                        new_wav_file += wave_id
                        new_wav_file += fmt_chunk_id
                        new_wav_file += struct.pack('>I', fmt_chunk_size)
                        new_wav_file += struct.pack('>H', audio_format)
                        new_wav_file += struct.pack('>H', num_channels)
                        new_wav_file += struct.pack('>I', sample_rate)
                        new_wav_file += struct.pack('>I', byte_rate)
                        new_wav_file += struct.pack('>H', block_align)
                        new_wav_file += struct.pack('>H', bits_per_sample)
                        new_wav_file += data_chunk_id
                        new_wav_file += struct.pack('>I', data_chunk_size)
                        new_wav_file += pcm_data_bytes
                        
                        wav_file = new_wav_file
                        ...
                        with io.BytesIO(wav_file) as f:
                            try:
                                with wave.open(f, 'rb') as wf:
                                    ...
                                    pcm_data.append(wav_pcm_data)
                                    combined_pcm_data += wav_pcm_data
                            except Exception as e:
                                logger.bind(tag=TAG).error(f"解析WAV文件失败: {e}")
                    else:
                        logger.bind(tag=TAG).warning(f"不支持的音频格式，文件头: {wav_file[:12]}")
            elif audio_format == "pcm":
                # 处理PCM格式：直接使用提供的PCM数据
                pcm_data = opus_data
                combined_pcm_data = b"" .join(pcm_data)
            else:
                # 处理其他格式（如opus）：解码为PCM数据
