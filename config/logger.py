...
def llm_asr_tts_filter(record):
    """只允许LLM、ASR和TTS相关的日志通过"""
    tag = record["extra"].get("tag", record["name"]).lower()
    # 检查tag是否包含llm、asr或tts
    return any(keyword in tag for keyword in ["llm", "asr", "tts", "fastapi_server"])
...
def setup_logging():
    ...
    if not _logger_initialized:
        ...
        # 输出到控制台 - 只显示LLM、ASR和TTS相关的日志
        logger.add(sys.stdout, format=log_format, level=log_level, filter=lambda record: llm_asr_tts_filter(record) and formatter(record))

        # 输出到文件 - 统一目录，按大小轮转
        # 日志文件完整路径
        log_file_path = os.path.join(log_dir, log_file)

        # 添加日志处理器 - 只显示LLM、ASR和TTS相关的日志
        logger.add(
            log_file_path,
            format=log_format_file,
            level=log_level,
            filter=lambda record: llm_asr_tts_filter(record) and formatter(record),
