...
from core.fastapi_server import FastAPIServer
...
async def main():
    try:
        ...
        logger.bind(tag=TAG).warning(f"ffmpeg检查失败: {e}")
        logger.bind(tag=TAG).info("继续运行，部分音频功能可能受限")
    config = load_config()
    ...
    # 启动 HTTP API 服务器
    api_server = FastAPIServer(config)
